...
- **다음 단계:** QA 시나리오 실행, 알림 정책 튜닝, 배포 전 통합 리허설, `custom-profile` Push Token DELETE/Hub unlike DELETE 재검증
...
- **Push Token & 멤버 사이트 관리**  
  - 허브 프로필 페이지(`/profile`)는 `custom-profile/v1/push-token` (GET/POST/DELETE) REST 엔드포인트로 토큰을 발급·재발급·삭제한다.  
  - DELETE 호출은 `package/headless-theme/inc/custom-auth.php`의 CORS 헤더(`Access-Control-Allow-Methods: GET, POST, DELETE, PUT, PATCH, OPTIONS`)가 허용하는 도메인(`http://localhost:3000`, `https://wpyvr.bitebuddy.ca`)에서만 동작한다.  
  - 토큰 삭제 후에는 `wp_user_profiles.hub_connected` 플래그가 0으로 리셋되며, 플러그인에 새 토큰을 교체해야 한다.
- **트렌드 관리 규칙**  
...
| Firebase 토큰 검증 실패 403 | ✅ 완료 | 2025-11-19 `wp_hub_push_logs.id=11` (403) + hub.log 기록 확인 |
| Hub Incoming Posts 맵핑/Publish 플로우 | ☐ 예정 | 저장/Publish Now 두 경로 모두 수행 |
| 좋아요/댓글/트렌드 지표 | ⚠️ 재검증 | `custom-auth.php`에서 DELETE 허용 → 브라우저 unlike 재시도 후 hot score 계산 확인 |
| Push Token 라이프사이클 (GET/POST/DELETE) | ⚠️ 재검증 | `/custom-profile/v1/push-token` DELETE CORS 허용 확인 및 revoke 성공 응답(200) 확인 |
